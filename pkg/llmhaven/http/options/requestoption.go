...
import (
	...
	"github.com/y0ug/ai-helper/pkg/llmhaven/http/config"
)
...
func WithJSONSet(key string, value interface{}) RequestOption {
	return func(r *config.RequestConfig) (err error) {
		...
		return fmt.Errorf(
			"cannot use WithJSONSet on a body that is not serialized as *bytes.Buffer",
		)
	}
}
...
func WithJSONDel(key string) RequestOption {
	return func(r *config.RequestConfig) (err error) {
		...
		return fmt.Errorf(
			"cannot use WithJSONDel on a body that is not serialized as *bytes.Buffer",
		)
	}
}
