title: "Package names - The Go Programming Language"
source: https://go.dev/blog/package-names
date: 2025-01-17 11:02
tags: 
description: How to name your packages.
...
Effective Go provides [guidelines][2] for naming packages, types, functions, and variables. This article expands on that discussion and surveys names found in the standard library. It also discusses bad package names and how to fix them.
...
Just as types in different packages can have the same name without ambiguity, packages in different directories can have the same name. For example, [runtime/pprof][4] provides profiling data in the format expected by the [pprof][5] profiling tool, while [net/http/pprof][6] provides HTTP endpoints to present profiling data in this format. Client code uses the package path to import the package, so there is no confusion. If a source file needs to import both `pprof` packages, it can [rename][7] one or both locally. When renaming an imported package, the local name should follow the same guidelines as package names (lower case, no `under_scores` or `mixedCaps`).
...
- [Effective Go][8]
- [How to Write Go Code][9]
- [Organizing Go Code (2012 blog post)][10]
- [Organizing Go Code (2014 Google I/O talk)][11]

**Next article:** [Testable Examples in Go][12]  
**Previous article:** [Errors are values][13]  
**[Blog Index][14]**

  [1]: https://go.dev/blog/
  [2]: https://go.dev/doc/effective_go.html#names
  [3]: https://go.dev/doc/code.html#GOPATH
  [4]: https://go.dev/pkg/runtime/pprof
  [5]: https://github.com/google/pprof
  [6]: https://go.dev/pkg/net/http/pprof
  [7]: https://go.dev/ref/spec#Import_declarations
  [8]: https://go.dev/doc/effective_go.html
  [9]: https://go.dev/doc/code.html
  [10]: https://go.dev/blog/organizing-go-code
  [11]: https://go.dev/talks/2014/organizeio.slide
  [12]: https://go.dev/blog/examples
  [13]: https://go.dev/blog/errors-are-values
  [14]: https://go.dev/blog/all
