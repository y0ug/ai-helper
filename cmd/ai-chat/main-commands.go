...
func mainMenuCommands(app *console.Console) console.Commands {
	return func() *cobra.Command {
		...
		backupCmd.Flags().
			Bool("dry-run", false, "Perform a dry run without actually creating the backup")
		...
		renameCmd.Flags().
			BoolP("force", "f", false, "Force rename, even if the new name already exists")
		renameCmd.Flags().
			BoolP("preserve-extension", "p", false, "Preserve the file extension while renaming")
		...
		deployCmd.Flags().
			Bool("clean", false, "Perform a clean deployment, removing previous versions")
